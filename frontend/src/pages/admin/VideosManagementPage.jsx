...
export default function VideosManagementPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await api.get('/videos', { params: { page, limit: 10 } });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addNotification('Video dihapus', 'success');
  ...
  return (
    <div>
      <AdminPage title="Kelola Video">
        <DataTable columns={columns} data={data} loading={loading} error={error}
          pagination={pagination} onPageChange={setPage}
          onAdd={() => navigate('/admin/video/tambah')}
          onEdit={(row) => navigate(`/admin/video/edit/${row.id}`)}
